...
entities <- bind_rows(books_publishers, books_authors)
...
only_books <- tibble(source_1 = "book_author", source_2 = "book_author")
...
candidates <- calc_similarity(bigrams, method = "cosine", min_sim = min_sim) %>% 
  ...
  anti_join(only_books, by = c("source_1", "source_2"))
...
#         str_detect(source_1, "book") ~ paste0("book_author_", id_1),
#         #source_1 == "book" ~ paste0("book_", id_1),
#         TRUE ~ "no_source_sth_wrong"
#       ),
#     id_2 = 
#       case_when(
#         str_detect(source_2, "book") ~ paste0("book_publisher_", id_2),
#         str_detect(source_2, "book") ~ paste0("book_author_", id_2),
#         TRUE ~ "no_source_sth_wrong"
#       )
#   ) %>% 
...
import <- candidates %>% 
  mutate(entities = "books_publishers_books_authors", .before = id_1)
