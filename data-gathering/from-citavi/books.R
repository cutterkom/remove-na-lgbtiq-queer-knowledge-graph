...
df_person_book <- df_link_author_book %>%
  ...
  mutate(author1 = ifelse(is.na(MiddleName) & !is.na(FirstName), paste(FirstName, LastName), NA),
  ...
  mutate(author4 = ifelse(!is.na(first), paste(first, second), NA),
...
books_wide_raw <- df_book_prep %>% as_tibble() %>% janitor::clean_names()
...
books_wide <- books_wide_raw %>% 
  ...
  mutate(is_herausgeber = ifelse(str_detect(autor_in, "Hg."), 1, 0),
...
dm_raw <- dm(book_author, book_editor, book_location,
