...
or_rec_data <- rrefine::refine_export("entities_publication") %>% 
  select(id, factgrid, wikidata)
...
final_descriptions <- input %>% 
  ...
  mutate(
    Dde = ifelse(is.na(Dde), "Publikation im Zusammenhang mit LGBTIQ*-Geschichte", Dde),
    Den = ifelse(is.na(Den), "publication related to LGBTIQ* history", Den),
    Dfr = ifelse(is.na(Dfr), "publication liée à l'histoire des LGBTIQ*", Dfr),
    Des = ifelse(is.na(Des), "publicación relacionada con la historia del colectivo LGBTIQ*", Des)
  ) %>% 
...
write_wikibase(
  ...
  values = import$value,
  format = "csv",
  format.csv.file = "data-publishing/factgrid/data/pubs.csv",
