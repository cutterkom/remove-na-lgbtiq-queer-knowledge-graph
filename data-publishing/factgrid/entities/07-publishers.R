...
input <- entities_raw %>% 
  filter(org == 1, club == 0) %>% 
  inner_join(entities_per_type %>% filter(!is.na(publisher_book_id)) %>% distinct(id), by = "id") %>% 
  distinct(id, name)
  distinct(id, name) %>% 
...
# Wie viele schon in Factgrid?
el_matches %>% filter(external_id_type == "factgrid") %>% distinct(id, external_id) %>% nrow() / nrow(entities_raw)


# Entity Linking ----------------------------------------------------------

el <- input %>% 
  left_join(el_matches, by = "id") %>% 
  ...
  distinct(id, external_id, external_id_type)
...
wikidata <- 
  input %>% 
  left_join(el %>% 
              filter(external_id_type == "wikidata"),
            by = "id") %>% 
  left_join(rrefine::refine_export("entities_publishers") %>% 
              mutate(openrefine_wikidata = ifelse(id == "book_publisher_100", NA, openrefine_wikidata),
                     openrefine_wikidata = ifelse(id == "book_publisher_23", NA, openrefine_wikidata),
                     openrefine_wikidata = ifelse(id == "book_publisher_674", NA, openrefine_wikidata)
                     )
              , by = "id") %>% 
  mutate(is_same = external_id == openrefine_wikidata) %>% 
  select(id, qid = openrefine_wikidata) %>% 
  filter(!is.na(qid)) %>% 
  add_row(id = "book_publisher_100", qid = "Q2516698")
...
final_descriptions <- input %>% 
  ...
  mutate(
    Dde = ifelse(is.na(Dde), "Verlag", Dde),
    Den = ifelse(is.na(Den), "publisher", Den),
    Dfr = ifelse(is.na(Dfr), "Maison d'édition", Dfr),
    Des = ifelse(is.na(Des), "editorial", Des)
  )
...
gnd_ids <- input %>% 
  left_join(el_matches %>% filter(external_id_type == "gnd")) %>% 
  distinct(id, external_id) %>% 
  filter(!is.na(external_id)) %>% 
  rename(P76 = external_id)
...
input_statements <- input %>%
  ...
  left_join(final_labels %>% select(-name), by = "id") %>% 
  left_join(final_descriptions, by = "id") %>% 
  add_statement(statements, "instance_of_publisher") %>% 
  add_statement(statements, "research_project") %>% 
  add_statement(statements, "research_area") %>% 
  left_join(gnd_ids, by = "id") %>% 
  left_join(wikipedia_sitelinks) %>% 
  left_join(wikidata %>% distinct(id, Swikidatawiki = qid) %>% 
              mutate(Swikidatawiki = ifelse(!is.na(Swikidatawiki), paste0(config$import_helper$single_quote, Swikidatawiki, config$import_helper$single_quote), Swikidatawiki)), 
            by = "id")
...
write_wikibase(
  ...
  values = import$value,
  format = "csv",
  format.csv.file = "data-publishing/factgrid/data/publlishers.csv",
...
SELECT ?item ?itemLabel ?instanceOf ?instanceOfLabel WHERE {
  ...
  # ?item wdt:P2 wd:Q39680
   OPTIONAL { ?item wdt:P2 ?instanceOf }
}
'
...
factgrid_qids <- input_statements %>% distinct(id, Lde) %>% 
  left_join(query_res, by = c("Lde" = "itemLabel")) %>% 
  distinct(id, external_id)
...
long_for_quickstatements <- function(data) {
  data %>% 
    pivot_longer(cols = 2:last_col(), names_to = "property", values_to = "value") %>% 
    # fix helper with two instances_of:
    mutate(property = str_remove(property, "_.*")) %>% 
...
import <- factgrid_qids %>% 
  select(item = external_id, P728 = id) %>% 
