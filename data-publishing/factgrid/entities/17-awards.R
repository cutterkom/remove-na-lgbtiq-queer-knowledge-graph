...
or_rec_data <- rrefine::refine_export("entities_awards") %>% 
  select(id, gnd = lobid, wikidata)
...
final_descriptions <- input %>% 
  ...
  mutate(
    Dde = ifelse(is.na(Dde), "Auszeichnung im Zusammenhang mit LGBTIQ*-Geschichte", Dde),
    Den = ifelse(is.na(Den), "award related to LGBTIQ* history", Den),
    Dfr = ifelse(is.na(Dfr), "prix liée à l'histoire des LGBTIQ*", Dfr),
    Des = ifelse(is.na(Des), "Premio relacionada con la historia del colectivo LGBTIQ*", Des)
  ) %>% 
...
input_statements <- input %>% 
  ...
  mutate(
    ...
    P131 = ifelse(str_detect(item, "CREATE"), P131, NA_character_),
    P97 = ifelse(str_detect(item, "CREATE"), P97, NA_character_)
  )
...
write_wikibase(
  ...
  values = import$value,
  format = "csv",
  format.csv.file = "data-publishing/factgrid/data/awards.csv",
