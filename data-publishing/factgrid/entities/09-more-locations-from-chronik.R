...
final_descriptions <- input %>% 
  ...
  mutate(
    Dde = ifelse(is.na(Dde), "Ort im Zusammenhang mit LGBT-Geschichte", Dde),
    Den = ifelse(is.na(Den), "Place related to LGBT history", Den),
    Dfr = ifelse(is.na(Dfr), "Lieu en rapport avec l'histoire LGBT", Dfr),
    Des = ifelse(is.na(Des), "Situarse en el contexto de la historia LGBT", Des)
  ) %>% 
...
wikidata_instances %>% clipr::write_clip()
...
write_wikibase(
  ...
  values = import$value,
  format = "csv",
  format.csv.file = "data-publishing/factgrid/data/loc.csv",
