...
save_decision <- function(candidates, decision, ...) {
  ...
  if (decision == "positive") {
    if (input$keep_candidate_2_label == TRUE) {
      query <- paste0("UPDATE ", db_tbl_er_candidates, " SET decision = '", this_decision, "', keep_label_2 = 1 WHERE id =", candidates$id, ";")
    } else {
...
```{r}
entities_list <- get_er_entities() %>% filter(entities != "manual") %>% set_names(get_er_entities() %>% filter(entities != "manual"))
selectInput("entities", label = "Entities", choices = entities_list)
```
