...
comparison_raw <- eventReactive(input$button_start_input_properties, {
  ...
  if(is.null(input$input_properties)) {
    ...
    data <- get_comparison(input_properties() %>% distinct(fg_property), input$item_filter_property, input$item_filter_value)
...
DT::renderDataTable(
  ...
  in_both() %>% 
    left_join(input_properties() %>% distinct(fg_property_id, fg_property_type)) %>% 
    # link for items
