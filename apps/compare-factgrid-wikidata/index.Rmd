...
all_properties <- all_properties_raw %>% 
  filter(str_detect(fg_property_type, "WikibaseItem|String|Time|Quantity|CommonsMedia|ExternalId|Url|CommonsMedia")) 
...
observeEvent(input$button_start_property_by_type, {
  ...
  insertUI(
      ...
      where = "afterEnd",
      ui = checkboxGroupInput("input_properties", label = "Available properties:", choices = get_properties_list_by_type(all_properties, input$property_by_type))
    )
})
```
