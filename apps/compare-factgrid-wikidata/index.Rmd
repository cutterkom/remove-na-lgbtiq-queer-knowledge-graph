...
comparison_raw <- eventReactive(input$button_start_person_properties, {
  ...
  if(is.null(input$person_properties)) {
    ...
    # fg_items <- filter_items(input$item_filter)
    # print(fg_items)
    print(input$item_filter)
...
output$download_wikidata_import_UI <- renderUI( {
  downloadButton("download_wikidata_import", "Download data for Wikidata import")
})
...
output$download_factgrid_import_UI <- renderUI( {
  downloadButton("download_factgrid_import", "Download data for factgrid import")
})
